...
PDF 논문을 업로드하면 서버에서 텍스트를 추출하고, OpenAI API로 구조화된 요약 보고서를 만든 뒤, Semantic Scholar 검색 결과 기반으로 유사 논문을 추천하는 React + Express MVP입니다. 최종 결과는 화면에서 확인하거나 요약 보고서 PDF로 다운로드할 수 있습니다.
...
- 보안: OpenAI API 키는 Express 서버 환경변수로만 사용하고 프론트엔드 번들에 포함하지 않음
...
5. 브라우저에서 Vite 개발 서버를 엽니다.

   ```text
   http://localhost:5173
   ```

Express API 서버는 기본적으로 `http://localhost:4000`에서 실행됩니다.
...
| `CLIENT_ORIGIN` | 아니오 | CORS 허용 프론트엔드 주소, 기본값 `http://localhost:5173` |
| `VITE_API_BASE_URL` | 아니오 | 클라이언트가 호출할 API 주소, 기본값 `http://localhost:4000/api` |
...
4. 서버가 제목과 핵심 키워드를 기반으로 Semantic Scholar에서 유사 논문을 검색합니다.
...
```bash
npm run dev      # 클라이언트와 서버 개발 서버 동시 실행
npm run build    # 클라이언트 프로덕션 빌드
npm run lint     # 클라이언트 ESLint 및 서버 문법 검사
