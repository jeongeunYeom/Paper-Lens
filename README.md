...
- PDF 다운로드: 요약 PDF에는 영문/국문 요약과 구조화된 요약만 포함하고, 영문 논문의 한국어 번역본은 별도의 Figure 보존 번역 PDF로 생성
...
- 요약 PDF는 `Paper Lens 논문 분석 보고서` 제목, 기본 정보 카드, 영문/국문 요약, 1~7번 요약 본문 섹션 순서로 생성됩니다. 한국어 전체 번역본, API 토큰 사용량, 유사 논문 추천 목록은 요약 PDF에 포함하지 않습니다. 영문 논문 전체 번역본은 별도 `Figure 보존 번역 PDF`로 제공합니다.
...
| `MAX_TRANSLATION_INPUT_CHARS` | 아니오 | 참고문헌 제외 후 번역에 사용할 최대 원문 글자 수, 기본값 `120000` |
...
7. `요약 PDF 다운로드` 버튼으로 요약 보고서 PDF를 다운로드합니다. 다운로드 파일명은 `논문제목_요약.pdf` 형식이며, 이 파일에는 영문/국문 요약만 포함하고 한국어 전체 번역본은 넣지 않습니다. 서버에 Poppler가 설치되어 있고 영문 논문 번역본이 생성되면 별도 `Figure 보존 번역 PDF`도 함께 받을 수 있습니다.
