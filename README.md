...
| `MAX_TRANSLATION_INPUT_CHARS` | 아니오 | 참고문헌 제외 후 번역에 사용할 최대 원문 글자 수, 기본값 `30000` |
| `TRANSLATION_CHUNK_SIZE` | 아니오 | 번역 API 호출당 원문 조각 크기, 기본값 `6000` |
...
7. `요약 PDF 다운로드` 버튼으로 보고서 PDF를 다운로드합니다. 다운로드 파일명은 `논문제목_요약.pdf` 형식으로 지정되며, PDF 보고서에는 번역본과 영문/국문 요약을 포함하고 API 토큰 사용량과 유사 논문 추천 목록은 제외합니다.
...
- 현재 번역본은 `pdf-parse`로 추출한 텍스트 기반입니다. 원본 PDF에 수록된 figure 이미지를 원본과 완전히 동일하게 재배치/복제하려면 별도의 레이아웃 보존 번역 파이프라인이 필요합니다. 구현 방향은 [`docs/figure-preserving-translation.md`](docs/figure-preserving-translation.md)를 참고하세요.
