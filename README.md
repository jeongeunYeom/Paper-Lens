...
OpenAI API를 사용한 요약은 응답의 `usage` 값을 읽어 입력 토큰, 출력 토큰, 총 토큰을 결과 화면과 PDF 보고서에 표시합니다. 예상 비용은 아래 공식으로 계산합니다.
...
6. `요약 PDF 다운로드` 버튼으로 보고서 PDF를 다운로드합니다. PDF 보고서에는 요약 본문만 포함하며, API 토큰 사용량과 유사 논문 추천 목록은 제외합니다.
